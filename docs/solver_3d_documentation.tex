...
\hne'(-1) &= L\,\code{dne_dx_bc}/n_0 \quad\text{(Neumann flux on ds(1))}.
\end{align*}
...
\[
\hne(0) = 1,\quad \hne'(0) = L\,\code{dne_dx_bc}/n_0,\quad
...
\begin{description}[style=nextline]
  ...
  \item[\code{dne_dx_bc} (float or None)] SI Neumann slope (m$^{-4}$),
  imposed at $x_{\mathrm{inner}}$ when \code{ne_grad_bc_loc="inner"} and at
  the separatrix when \code{ne_grad_bc_loc="outer"} (this is how Saarelma
  Eq.~(20) is fed in). Left \code{None} it is read off the gradient of
  \code{state.n_e} at that location.
  \item[\code{ne_inner} (float or None)] Pedestal-top density (m$^{-3}$)
  used only to shape the initial guess. Left \code{None} it is read off
  \code{state.n_e} at $x_{\mathrm{inner}}$. Both are resolved by
