...
pedestal-top slope (or \code{dne_dx_neginf} if given), because it is a
...
\code{first_step="skip"}, a shallower \code{dne_dx_bc}, or an inner
...
\begin{description}[style=nextline]
  ...
  inner boundary free.
  \item[\code{dne_dx_bc} (float or None)] SI Neumann slope (m$^{-4}$, must
  be negative), imposed at $x_{\mathrm{inner}}$ when
  \code{ne_grad_bc_loc="inner"} and at the separatrix when
  \code{ne_grad_bc_loc="outer"} (this is how Saarelma Eq.~(20) is fed in).
  Left \code{None} it is read off the gradient of \code{state.n_e} at that
  location (resolved by \code{bc_ig_helpers.resolve_ne_bcs}).
  \item[\code{dne_dx_neginf} (float or None)] Saarelma's constant of
  integration $C$ in the source term, in either \code{ne_grad_bc_loc} mode.
  Left \code{None} it is read off the gradient of \code{state.n_e} at
  $x_{\mathrm{inner}}$ (resolved by
  \code{bc_ig_helpers.resolve_integration_constant}).
...
$N'(0)$ matches \code{dne_dx_bc} (see Section~\ref{sec:bcs} for why a
